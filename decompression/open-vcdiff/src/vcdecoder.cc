...
namespace open_vcdiff
{
  ...
  public:
    ...
    TextInfo *buffer = (TextInfo *)malloc(10485760); // store preprocess
      ...
      ParseableChunk *data)
  {
    ...
    default:
      ...
      if ((vcdiff_version_code_ != 0x00) && // Draft standard VCDIFF (RFC 3284)
          (vcdiff_version_code_ != 'S'))
      { // Enhancements for SDCH protocol
        VCD_ERROR << "Unrecognized VCDIFF format version" << VCD_ENDL;
        return RESULT_ERROR;
      }
      FALLTHROUGH_INTENDED;
    case 3:
      if (header->header3 != 0xC4)
      { // magic value 'D' | 0x80
        wrong_magic_number = true;
      }
      FALLTHROUGH_INTENDED;
    case 2:
      if (header->header2 != 0xC3)
      { // magic value 'C' | 0x80
        wrong_magic_number = true;
      }
      FALLTHROUGH_INTENDED;
    case 1:
      if (header->header1 != 0xD6)
      { // magic value 'V' | 0x80
        wrong_magic_number = true;
      }
      FALLTHROUGH_INTENDED;
    case 0:
      if (wrong_magic_number)
      {
        VCD_ERROR << "Did not find VCDIFF header bytes; "
                     "input is not a VCDIFF delta file"
                  << VCD_ENDL;
        return RESULT_ERROR;
      }
      if (data_size < sizeof(DeltaFileHeader))
      ...
      ParseableChunk *data)
  {
    ...
    if (!custom_code_table_.get())
    {
      VCD_DFATAL << "Internal error:  custom_code_table_decoder_ is set,"
                    " but custom_code_table_ is NULL"
                 << VCD_ENDL;
      return RESULT_ERROR;
    }
    ...
    else
    {
      ...
      if (addresses_for_copy_.End() != header_parser->EndOfDeltaWindow())
      {
        VCD_ERROR << "The end of the instructions section "
                     "does not match the end of the delta window"
                  << VCD_ENDL;
        return RESULT_ERROR;
      }
    }
  ...
  VCDiffResult VCDiffDeltaFileWindow::DecodeAdd(size_t size)
  {
    ...
    unsigned char* word = (unsigned char*)data_for_add_and_run_.UnparsedData();
    int count = size;
		/*while (count > 0)
		{
			parent_->buffer[parent_->index].cmd.cmd = 1;
			parent_->buffer[parent_->index].cmd.token = *word;
			parent_->index += 1;
			count--;
			word++;
		}*/
  ...
  VCDiffResult VCDiffDeltaFileWindow::DecodeRun(size_t size)
  {
    ...
    unsigned char* token = (unsigned char*)*data_for_add_and_run_.UnparsedData();
    parent_->buffer[parent_->index].cmd.cmd = 4;
    parent_->buffer[parent_->index].cmd.token = *token;
    parent_->index++;
    return RESULT_SUCCESS;
  }
                                                 ...
                                                 unsigned char mode)
  {
    ...
    if ((address + size) <= source_segment_length_)
    {
      ...
      parent_->buffer[parent_->index].cmd.cmd = 2;
      parent_->buffer[parent_->index].cmd.token = 0;
      parent_->index += 1;

      parent_->buffer[parent_->index].index = address;
      parent_->index += 1;
      
      parent_->buffer[parent_->index].len = size;
      parent_->index += 1;
    ...
    while (size > (target_bytes_decoded - address))
    {
      ...
      /*if (flag++ == 0)
      {
        parent_->buffer[parent_->index].cmd.cmd = 3;
        parent_->buffer[parent_->index].cmd.token = 0;

        parent_->index += 1;
        parent_->buffer[parent_->index].dist = target_bytes_decoded - address;
        parent_->index += 1;
        parent_->buffer[parent_->index].len = size;
        parent_->index += 1;
      }*/
  ...
  int VCDiffDeltaFileWindow::DecodeBody(ParseableChunk *parseable_chunk)
  {
    ...
    if (has_checksum_ &&
        (ComputeAdler32(target_window_start, target_window_length_) != expected_checksum_))
    {
      VCD_ERROR << "Target data does not match checksum; this could mean "
                   "that the wrong dictionary was used"
                << VCD_ENDL;
      return RESULT_ERROR;
    }
      ...
      ParseableChunk *parseable_chunk)
  {
    if (!parent_)
    {
      VCD_DFATAL << "Internal error: VCDiffDeltaFileWindow::DecodeWindow() "
                    "called before VCDiffDeltaFileWindow::Init()"
                 << VCD_ENDL;
      return RESULT_ERROR;
    }
    if (!found_header_)
    ...
    else
    {
      ...
      if (!IsInterleaved())
      {
        VCD_DFATAL << "Internal error: Resumed decoding of a delta file window"
                      " when interleaved format is not being used"
                   << VCD_ENDL;
        return RESULT_ERROR;
      }
