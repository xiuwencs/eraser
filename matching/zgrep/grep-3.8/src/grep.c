...
}; /* For when a signed size is wanted.  */
  ...
  else
  {
    ...
    else
    {
      ...
      if (usable_st_size(st))
      {
        ...
        ptrdiff_t a;
        if (0 <= to_be_read && INT_ADD_OK(to_be_read, save + min_after_buflim, &a))
...
grepdesc(int desc, bool command_line)
{
  ...
  t6 = th.tv_sec * 1000 + th.tv_usec / 1000;
  if (count_matches)
  {
    if (out_file)
    {
  ...
  gettimeofday(&th, NULL);
  t7 = th.tv_sec * 1000 + th.tv_usec / 1000;
  fprintf(stderr, "t6 - t5 = %ld, t7 - t4 = %ld\n", t6 - t5, t7 - t4);
closeout:
...
int main(int argc, char **argv)
{
  ...
  gettimeofday(&th, NULL);
  t4 = th.tv_sec * 1000 + th.tv_usec / 1000;
  if (matcher < 0)
  ...
  skip_empty_lines = ((execute(compiled_pattern, eolbytes + 1, 1,
                               &match_size, NULL) == 0) == out_invert);
  gettimeofday(&th, NULL);
  t5 = th.tv_sec * 1000 + th.tv_usec / 1000;
