...
#define GNU_STANDARD 1
    ...
    if (to_stdout && ((synchronous && fdatasync(STDOUT_FILENO) != 0 && errno != EINVAL) || close(STDOUT_FILENO) != 0) && errno != EBADF)
        write_error();
    do_exit(exit_code);
}
...
{
    if (!force && !list && (presume_input_tty || isatty(decompress ? STDIN_FILENO : STDOUT_FILENO)))
    {
...
/* Don't bother opening directories on older systems that
   lack openat and unlinkat.  It's not worth the porting hassle.  */
    ...
    enum
    {
        try_opening_directories = true
    };
#else
    enum
    {
        try_opening_directories = false
    };
#endif
...
local void treat_file(iname) char *iname;
{
    ...
    fprintf(stderr, "t2 - t1 = %ld , t3 - t1 = %ld\n", t2 - t1, t3 - t1);
    ...
    if (!to_stdout)
    {
        ...
        if ((synchronous && ((0 <= dfd && fdatasync(dfd) != 0 && errno != EINVAL) || (fsync(ofd) != 0 && errno != EINVAL))) || close(ofd) != 0)
            write_error();
...
local int create_outfile()
{
    ...
    int flags = (O_WRONLY | O_CREAT | O_EXCL | (ascii && decompress ? 0 : O_BINARY));
...
char *name;
{
    ...
    {
        int s = strlen(*suf);
        if (slen > s && !ISSLASH(suffix[slen - s - 1]) && strequ(suffix + slen - s, *suf))
        {
            match = name + nlen - s;
            break;
        }
...
#endif
    int fd;
    int open_flags = (O_RDONLY | O_NONBLOCK | O_NOCTTY | (ascii && !decompress ? 0 : O_BINARY));
...
do_list(int method)
{
    ...
    if (verbose)
    {
        static char const month_abbr[][4] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
local void copy_stat(ifstat) struct stat *ifstat;
{
    ...
    restoring = (decompress && 0 <= time_stamp.tv_nsec && !(timespec[1].tv_sec == time_stamp.tv_sec && timespec[1].tv_nsec == time_stamp.tv_nsec));
    if (restoring)
 ...
 */
local void treat_dir(fd, dir) int fd;
char *dir;
{
    DIR *dirp;
