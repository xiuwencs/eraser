    ...
    while (queue_count(queue) > 0)
    {
        ...
        for (i = 0; i < ALPHABET_SIZE; i++)
        {
                ...
                //如果节点r读取字符i后下一个状态存在
            {
                ...
                queue_add(queue, s); //节点r的后继状态节点s加入队列
                fs = acsm->acsmStateTable[r].FailState; // s的父节点（也就是r）的failure指针t

                /*
                 *  Locate the next valid state for 'i' starting at s
                 */
                 //只要节点fs读取字符i没有后继 那么更新fs
...
Convert_NFA_To_DFA(ACSM_STRUCT* acsm)
{
    ...
    while (queue_count(queue) > 0)
    {
        ...
        {
            if ((s = acsm->acsmStateTable[r].NextState[i]) != ACSM_FAIL_STATE)
            {//如果节点r读取字符i到达状态s 加入状态s节点
                queue_add(queue, s);
            }
            else
            {//如果节点r读取字符i无法转化 则将其后继状态置为其failure指针读取字符i的后继状态
...
acsmAddPattern(ACSM_STRUCT* p, unsigned char* pat, int n, int nocase,
    int offset, int depth, int negative)
{
    ACSM_PATTERN* plist;
    plist = (ACSM_PATTERN*)AC_MALLOC(sizeof(ACSM_PATTERN));//为模式结构分配空间
    MEMASSERT(plist, "acsmAddPattern");
    plist->patrn = (unsigned char*)AC_MALLOC(n);
    memcpy(plist->patrn, pat, n); //保存原有模式
...
acsmCompile(ACSM_STRUCT* acsm)
{
    ...
    for (plist = acsm->acsmPatterns; plist != NULL; plist = plist->next) //遍历模式串集
    {
    ...
    void* data, int* current_state)
{
    ...
    for (; T < Tend; T++)
    {
        ...
        {
            mlist = StateTable[state].MatchList;//对应匹配的模式串
    ...
    for (k = 0; k < acsm->acsmMaxStates; k++)
    {
        ...
        for (i = 0; i < ALPHABET_SIZE; i++)
        {
            ...
            if (next != ACSM_FAIL_STATE) {
                ...
                if (isprint(i)) //判断i是否是可打印字符
                    printf("state %3d read %3c  transfer to %5d\n", k, i, next);
                else
                    printf("state %3d read %3d  transfer to %5d\n", k, i, next);
            }
        }
        printf("\n");
    }
